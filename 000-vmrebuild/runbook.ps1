    ...
    if (($status -eq "Activated") -or ($status -eq "Fired"))
    {
        ...
        if ($ResourceType -eq "microsoft.operationalinsights/workspaces")
        {
            ...
            if (!($alertCI -eq $null)) {
				...
				$url="https://management.azure.com/subscriptions/$SubscriptionId/resourceGroups/$ResourceGroupName/providers/microsoft.insights/scheduledQueryRules/AR-HeartbeatsLost-RebuildableVM01?api-version=2018-04-16"
				$rBody = @{
					'properties' = @{
						'enabled' = 'false'
					}
